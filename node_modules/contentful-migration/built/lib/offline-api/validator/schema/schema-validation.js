    ...
    return cleanErrors.map((details) => {
        ...
        if (prop.startsWith('allowedResources')) {
            if (path.length > 3) {
                ...
                return {
                    type: 'InvalidPayload',
                    message: errors_1.default.field.allowedResources.INVALID_RESOURCE_PROPERTY(field.id, path[2], error)
                };
            }
            if (type === 'object.base') {
                ...
                return {
                    type: 'InvalidPayload',
                    message: errors_1.default.field.allowedResources.INVALID_RESOURCE(field.id, context.key, actualType)
                };
            }
            if (type === 'array.min') {
                return {
                    type: 'InvalidPayload',
                    message: errors_1.default.field.allowedResources.TOO_FEW_ITEMS(field.id)
                };
            }
            if (type === 'array.max') {
                return {
                    type: 'InvalidPayload',
                    message: errors_1.default.field.allowedResources.TOO_MANY_ITEMS(field.id)
                };
            }
            if (type === 'array.unique') {
                return {
                    type: 'InvalidPayload',
                    message: errors_1.default.field.allowedResources.DUPLICATE_SOURCE(field.id, context.value.source)
                };
            }
        }
        if (type === 'any.required') {
